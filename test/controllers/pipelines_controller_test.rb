...
class PipelinesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pipeline" do
    assert_difference("Pipeline.count") do
      post pipelines_url, params: { pipeline: { desc: @pipeline.desc, description: @pipeline.description, name: @pipeline.name } }
    end
  ...
  test "should update pipeline" do
    patch pipeline_url(@pipeline), params: { pipeline: { desc: @pipeline.desc, description: @pipeline.description, name: @pipeline.name } }
